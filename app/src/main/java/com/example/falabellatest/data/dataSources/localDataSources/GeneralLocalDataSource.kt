...
class GeneralLocalDataSource(
    ...
    fun getLogin(username: String, password: String): LoginEntry? {
        return sharedPreferences.getString(username, null)?.let { LoginEntry.create(it, object : TypeToken<LoginEntry>() {}.type).let { login -> if(login.comparePassword(password)) login else null  } }
    }
